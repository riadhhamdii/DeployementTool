	...
	var methods = {
					...
					if (!$(this).hasClass("ms2side__hide")) {
						if ($(this).hasClass("AddOne")) {
							...
							rightSel.find("option:selected").each(function(i, selected){
								//alert(leftSel.find('option:first').attr('p'));return false;
								$p = parseInt($(this).attr('p'));
								$pos = $p -1;
								leftSel.find('option').each(function(){
										if((parseInt($(this).attr('p')) < $p) && (parseInt($(this).attr('p')) > $pos)) {
											$pos = parseInt($(this).attr('p'));
										}
								});
								$new = $(this).clone();
								$(this).remove();
								if(($pos == ($p-1)) || (leftSel.children().length == 0)){
									leftSel.prepend($new);
								} else {
									leftSel.find('option[p="'+$pos+'"]').after($new);
								}								
								//~ $(this).remove().appendTo(leftSel);
		...
		destroy : function( ) {
			return this.each(function () {
				...
				if (!data)
					return;
				if(el.show().next().hasClass('ms2side__div')) {
					el.show().next().remove();
				}
			});
		},
		destroy1 : function( option) {
			var	el = $(this);
			el.find('option[value='+option.value+']').remove();
		},
		addOption : function(options) {
			...
			return this.each(function () {
				...
				var	strEl = "<option p='"+oAddOption.id+"' value='" + oAddOption.value + "' " + (oAddOption.selected ? "selected" : "") + " >" + oAddOption.name + "</option>";
...
$.fn.validateMax = function(filter, returnChecked) {		
	o = this;
	o.find('option').click(function(){
			if(o.find('option:selected').length > filter.Max) {
				$.validationEngine.buildPrompt($(this),"le nombre maximale est "+filter.Max ,"error");
				$(this).parent().find('option').click(function(){$.validationEngine.closePrompt(".formError",true)});
			}
	})
} 
...
$.fn.validateMin = function(filter, returnChecked) {		
		var o = this;
		o.find('option').click(function(){
				if(o.find('option:selected').length < filter.Min) {
