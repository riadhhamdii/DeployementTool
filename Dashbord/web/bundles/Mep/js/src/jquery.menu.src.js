...
jQuery.fn.scrollLeftM = function() {
...
jQuery.fn.offsetM = function(options, returnObject) {
...
(function($){
	...
	function showMenuLI() {
		...
		var sx = $(window).scrollLeftM()
		var sy = $(window).scrollTopM();
		...
		var thisOffset 		= $(this).offsetM({border: false});
	...
	// Left vertical
		...
		if (settings.onAnimate) {
			...
			var left = 40+ $(ul).parent().width()
			$(ul).css({left: left, top: menuPosition.y1}).show();
		}
