...
;(function($) {
	var tmp, loading, overlay, wrap, outer, content, close, title, nav_left, nav_right,
		...
		_show = function() {
			...
			content
				...
				.html( tmp.contents() );
			if(currentOpts.left != "" ) {
				final_pos.left = currentOpts.left;
			}
			if(currentOpts.top != "" ) {
				final_pos.top = currentOpts.top;
			}
		...
		_process_title = function() {
			...
			title
				.addClass('lightbox-title-' + currentOpts.titlePosition)
				.html( titleStr )
				.appendTo( 'body' )
				.show();
			...
			if (currentOpts.enableEscapeButton || currentOpts.enableKeyboardNav) {
				$(document).bind('keydown.fb', function(e) {
					if (e.keyCode == 27 && currentOpts.enableEscapeButton) {
						...
						e.preventDefault();
						$.lightbox[ e.keyCode == 37 ? 'prev' : 'next']();
					}
				});
			}
		...
		_finish = function () {
			...
			if (currentOpts.hideOnContentClick)	{
				content.bind('click', $.lightbox.close);
			}
			...
			if (currentOpts.type == 'iframe') {
				$('<iframe id="lightbox-frame" name="lightbox-frame' + new Date().getTime() + '" frameborder="0" hspace="0" ' + ($.browser.msie ? 'allowtransparency="true""' : '') + ' scrolling="' + selectedOpts.scrolling + '" src="' + currentOpts.href + '"></iframe>').appendTo(content);
			}
		...
		if ($.isArray(obj)) {
			for (var i = 0, j = obj.length; i < j; i++) {
				if (typeof obj[i] == 'object') {
					$(obj[i]).data('lightbox', $.extend({}, opts, obj[i]));
				} else {
					obj[i] = $({}).data('lightbox', $.extend({content : obj[i]}, opts));
				}
			}
	...
	$.lightbox.next = function() {
		return $.lightbox.pos( currentIndex + 1);
	};
	...
	$.lightbox.init = function() {
		...
		outer = $('<div id="lightbox-outer"></div>')
			.append('<div class="lightbox-bg" id="lightbox-bg-n"></div><div class="lightbox-bg" id="lightbox-bg-ne"></div><div class="lightbox-bg" id="lightbox-bg-e"></div><div class="lightbox-bg" id="lightbox-bg-se"></div><div class="lightbox-bg" id="lightbox-bg-s"></div><div class="lightbox-bg" id="lightbox-bg-sw"></div><div class="lightbox-bg" id="lightbox-bg-w"></div><div class="lightbox-bg" id="lightbox-bg-nw"></div>')
			.appendTo( wrap );

		outer.append(
			content = $('<div id="lightbox-content"></div>'),
			close = $('<a id="lightbox-close"></a>'),
			title = $('<div id="lightbox-title"></div>'),

			nav_left = $('<a href="javascript:;" id="lightbox-left"><span class="fancy-ico" id="lightbox-left-ico"></span></a>'),
			nav_right = $('<a href="javascript:;" id="lightbox-right"><span class="fancy-ico" id="lightbox-right-ico"></span></a>')
		);

		close.click($.lightbox.close);
