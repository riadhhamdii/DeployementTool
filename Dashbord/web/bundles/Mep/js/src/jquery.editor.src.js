...
function elDialogForm(o) {
	...
	**/
	this.open = function() {
		...
		// 	}
		// });
		$(this).find('.ui-widget-header').remove('ui-widget-header');
...
elRTE.prototype.ui.prototype.buttons.image = function(rte, name) {
	...
	this.command = function() {
		...
		if (this.img.attr('src') && ($.trim(self.src.main.src.val()).length != 0 )){
		...
		$.each(this.src, function() {
			$.each(this, function() {
				if (this === self.src.main.src) {
					...
					this.bind('change', function(e) {if ($.trim(self.src.main.src.val()).length != 0 ) { self.updateDimesions(e);}});
				} else {
					this.bind('change', function() { if ($.trim(self.src.main.src.val()).length != 0 ) { self.updateImg(); }});
				}
			});
		});
