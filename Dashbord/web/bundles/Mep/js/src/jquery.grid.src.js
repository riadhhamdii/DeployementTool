...
$.widget("ui.multiselect", {
	_init: function() {
		...
		this.container = $('<div class="ui-multiselect ui-helper-clearfix-grid ui-widget-grid"></div>').insertAfter(this.element);
		this.count = 0; // number of currently selected options
		this.selectedContainer = $('<div class="selected"></div>').appendTo(this.container);
		this.availableContainer = $('<div class="available"></div>').appendTo(this.container);
		this.selectedActions = $('<div class="actions ui-widget-header-grid ui-helper-clearfix-grid"><span class="count">0 '+$.ui.multiselect.locale.itemsCount+'</span><a href="#" class="remove-all">'+$.ui.multiselect.locale.removeAll+'</a></div>').appendTo(this.selectedContainer);
		this.availableActions = $('<div class="actions ui-widget-header-grid ui-helper-clearfix-grid"><input type="text" class="search empty ui-widget-content-grid ui-corner-all-grid"/><a href="#" class="add-all">'+$.ui.multiselect.locale.addAll+'</a></div>').appendTo(this.availableContainer);
		this.selectedList = $('<ul class="selected connected-list"><li class="ui-helper-hidden-accessible-grid"></li></ul>').bind('selectstart', function(){return false;}).appendTo(this.selectedContainer);
		this.availableList = $('<ul class="available connected-list"><li class="ui-helper-hidden-accessible-grid"></li></ul>').bind('selectstart', function(){return false;}).appendTo(this.availableContainer);
	...
	_populateLists: function(options) {
		this.selectedList.children('.ui-element-grid').remove();
		this.availableList.children('.ui-element-grid').remove();
		this.count = 0;
	...
	_getOptionNode: function(option) {
		option = $(option);
		var node = $('<li class="ui-state-default-grid ui-element-grid" title="'+option.text()+'"><span class="ui-icon-os"/>'+option.text()+'<a href="#" class="action"><span class="ui-corner-all-grid ui-icon-os"/></a></li>').hide();
	...
	_applyItemState: function(item, selected) {
		if (selected) {
			if (this.options.sortable)
				item.children('span').addClass('ui-icon-arrowthick-2-n-s').removeClass('ui-helper-hidden').addClass('ui-icon-os');
			else
				item.children('span').removeClass('ui-icon-arrowthick-2-n-s').addClass('ui-helper-hidden').removeClass('ui-icon-os');
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var eg = $("<div class='ui-jqgrid ui-widget-grid ui-widget-content-grid ui-corner-all-grid'></div>");
		...
		$("<div class='loading ui-state-default-grid ui-state-active' id='load_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(gv);
		...
		addRowNum = function (pos,irow,pG,rN) {
			...
			return "<td role=\"gridcell\" class=\"ui-state-default-grid jqgrid-rownum\" "+prp+">"+v+"</td>";
		},
        ...
        setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all-grid'><span class='ui-icon-os ui-icon-seek-first'></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all-grid'><span class='ui-icon-os ui-icon-seek-prev'></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all-grid'><span class='ui-icon-os ui-icon-seek-next'></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all-grid'><span class='ui-icon-os ui-icon-seek-end'></span></td>";
			...
			$(document.body).append("<div id='testpg' class='ui-jqgrid ui-widget-grid ui-widget-content-grid' style='font-size:"+tdw+";visibility:hidden;' ></div>");
		...
		imgs = "<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+iac+" ui-state-disabled ui-icon-os ui-icon-triangle-1-n ui-sort-"+dir+"'></span>";
		imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+idc+" ui-state-disabled ui-icon-os ui-icon-triangle-1-s ui-sort-"+dir+"'></span></span>";
		for(i=0;i<this.p.colNames.length;i++){
			var tooltip = ts.p.headertitles ? (" title=\""+$.jgrid.stripHtml(ts.p.colNames[i])+"\"") :"";
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default-grid ui-th-column ui-th-"+dir+"'"+ tooltip+">";
		...
		$(grid.hDiv)
			...
			.addClass("ui-state-default-grid ui-jqgrid-hdiv")
			.append(hb);
		$(hb).append(hTable);
		hTable = null;
        ...
        if(ts.p.pager){
			...
			$(ts.p.pager).css({width: grid.width+"px"}).appendTo(eg).addClass('ui-state-default-grid ui-jqgrid-pager ui-corner-bottom');
		...
		.append("<span class='ui-icon-os ui-icon-circle-triangle-n'></span>").css((dir=="rtl"?"left":"right"),"0px") : "";
		$(grid.cDiv).append(arf).append("<span class='ui-jqgrid-title"+(dir=="rtl" ? "-rtl" :"" )+"'>"+ts.p.caption+"</span>")
		.addClass("ui-jqgrid-titlebar ui-widget-header-grid ui-corner-top ui-helper-clearfix-grid");
		...
		if( ts.p.toolbar[0] ) {
			...
			if(ts.p.toolbar[1]=="both") {
				...
				$(grid.uDiv).insertBefore(grid.hDiv).addClass("ui-userdata ui-state-default-grid").attr("id","t_"+this.id);
				$(grid.ubDiv).insertAfter(grid.hDiv).addClass("ui-userdata ui-state-default-grid").attr("id","tb_"+this.id);
				if(hg)  {$(grid.ubDiv).hide();}
			} else {
				$(grid.uDiv).width(grid.width).addClass("ui-userdata ui-state-default-grid").attr("id","t_"+this.id);
			}
		...
		if(ts.p.toppager) {
			...
			$(grid.topDiv).insertBefore(grid.hDiv).addClass('ui-state-default-grid ui-jqgrid-toppager').width(grid.width);
		...
		if(ts.p.footerrow) {
			...
			if(ts.p.rownumbers) { grid.footers[0].className = 'ui-state-default-grid jqgrid-rownum'; }
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(ni){
						...
						row += "<td role=\"gridcell\" aria-describedby=\""+t.p.id+"_rn\" class=\"ui-state-default-grid jqgrid-rownum\" "+prp+">0</td>";
					}
					if(gi) {
...
$.jgrid.extend({
	...
	GridUnload : function(){
		return this.each(function(){
			...
			if (this.p.pager) {
				$(this.p.pager).empty().removeClass("ui-state-default-grid ui-jqgrid-pager corner-bottom");
			}
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(i,n){
				...
				th = $("<th role='columnheader' class='ui-state-default-grid ui-th-column ui-th-"+$t.p.direction+"'></th>");
...
;(function($) {
	...
	$.fn.fmatter.actions = function(cellval,opts, rwd) {
		...
		if(op.editformbutton){
			...
			str =str+ "<div style='margin-left:8px;'><div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='ui-icon-os ui-icon-pencil'></span></div>";
		...
		if(op.delbutton) {
			...
			str = str+"<div title='"+$.jgrid.nav.deltitle+"' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' "+ocl+"><span class='ui-icon-os ui-icon-trash'></span></div>";
		}
		...
		str = str+"<div title='"+$.jgrid.edit.bSubmit+"' style='float:left;display:none' class='ui-pg-div ui-inline-save'><span class='ui-icon-os ui-icon-disk' "+ocl+"></span></div>";
		...
		str = str+"<div title='"+$.jgrid.edit.bCancel+"' style='float:left;display:none;margin-left:5px;' class='ui-pg-div ui-inline-cancel'><span class='ui-icon-os ui-icon-cancel' "+ocl+"></span></div></div>";
		return str;
	};
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {		
		...
		mw.className= "ui-widget-grid ui-widget-content-grid ui-corner-all-grid ui-jqdialog";
		mw.id = aIDs.themodal;
		var mh = document.createElement('div');
		mh.className = "ui-jqdialog-titlebar ui-widget-header-grid ui-corner-all-grid ui-helper-clearfix-grid";
		mh.id = aIDs.modalhead;
		$(mh).append("<span class='ui-jqdialog-title'>"+p.caption+"</span>");
		var ahr= $("<a href='javascript:void(0)' class='ui-jqdialog-titlebar-close ui-corner-all-grid'></a>")
		.hover(function(){ahr.addClass('ui-state-hover');},
			   function(){ahr.removeClass('ui-state-hover');})
		.append("<span class='ui-icon-os ui-icon-closethick'></span>");
		$(mh).append(ahr);
		if(rtlsup) {
			mw.dir = "rtl";
		...
		if(p.resize) {
			if($.fn.jqResize) {
				$(mw).append("<div class='jqResize ui-resizable-handle ui-resizable-se ui-icon-os ui-icon-gripsmall-diagonal-se ui-icon-grip-diagonal-se'></div>");
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		if(mopt.buttons.length > 0) {
			for(var i=0;i<mopt.buttons.length;i++) {
				...
				buttstr += "<a href='javascript:void(0)' id='"+mopt.buttons[i].id+"' class='fm-button ui-state-default-grid ui-corner-all-grid'>"+mopt.buttons[i].text+"</a>";
			}
		}
		...
		cnt += c_b ? "<div class='ui-widget-content-grid ui-helper-clearfix-grid' style='text-align:"+mopt.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a href='javascript:void(0)' id='closedialog' class='fm-button ui-state-default-grid ui-corner-all-grid'>"+c_b+"</a>"+buttstr+"</div>" :
			buttstr !== ""  ? "<div class='ui-widget-content-grid ui-helper-clearfix-grid' style='text-align:"+mopt.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'>"+buttstr+"</div>" : "";
		cnt += "</div>";
...
$.fn.jqFilter = function( arg ) {
	...
	return this.each( function() {
		...
		if(this.p.showQuery) {
			$(this).append("<table class='queryresult ui-widget-grid ui-widget-content-grid' style='display:block;max-width:440px;border:0px none;'><tbody><tr><td class='query'></td></tr></tbody></table>");
		}
		/*
		 *Perform checking.
		 *
		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			var table = $("<table class='group ui-widget-grid ui-widget-content-grid' style='border:0px none;'><tbody></tbody></table>");
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
				...
				bS  ="<a href='javascript:void(0)' id='"+fid+"_search' class='fm-button ui-state-default-grid ui-corner-all-grid fm-button-icon-right ui-reset'><span class='ui-icon-os ui-icon-search'></span>"+p.Find+"</a>",
				bC  ="<a href='javascript:void(0)' id='"+fid+"_reset' class='fm-button ui-state-default-grid ui-corner-all-grid fm-button-icon-left ui-search'><span class='ui-icon-os ui-icon-arrowreturnthick-1-w'></span>"+p.Reset+"</a>",
				bQ = "", tmpl="", colnm, found = false, bt, cmi=-1;
				if(p.showQuery) {
					bQ ="<a href='javascript:void(0)' id='"+fid+"_query' class='fm-button ui-state-default-grid ui-corner-all-grid fm-button-icon-left'><span class='ui-icon-os ui-icon-comment'></span>Query</a>";
				}
				if(!p.columns.length) {
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
				...
				// buttons at footer
				var bP = "<a href='javascript:void(0)' id='"+bp+"' class='fm-button ui-state-default-grid ui-corner-left'><span class='ui-icon-os ui-icon-triangle-1-w'></span></a>",
				bN = "<a href='javascript:void(0)' id='"+bn+"' class='fm-button ui-state-default-grid ui-corner-right'><span class='ui-icon-os ui-icon-triangle-1-e'></span></a>",
				bS  ="<a href='javascript:void(0)' id='sData' class='fm-button ui-state-default-grid ui-corner-all-grid'>"+p.bSubmit+"</a>",
				bC  ="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default-grid ui-corner-all-grid'>"+p.bCancel+"</a>";
				...
				if(rp_ge.checkOnSubmit || rp_ge.checkOnUpdate) {
					bS  ="<a href='javascript:void(0)' id='sNew' class='fm-button ui-state-default-grid ui-corner-all-grid' style='z-index:1002'>"+p.bYes+"</a>";
					bN  ="<a href='javascript:void(0)' id='nNew' class='fm-button ui-state-default-grid ui-corner-all-grid' style='z-index:1002'>"+p.bNo+"</a>";
					bC  ="<a href='javascript:void(0)' id='cNew' class='fm-button ui-state-default-grid ui-corner-all-grid' style='z-index:1002'>"+p.bExit+"</a>";
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				tdtmpl = "<td class='CaptionTD form-view-label ui-widget-content-grid' width='"+p.labelswidth+"'>&#160;</td><td class='DataTD form-view-data ui-helper-reset-os ui-widget-content-grid'>&#160;</td>", tmpl="",
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
				...
				// buttons at footer
				bP = "<a href='javascript:void(0)' id='"+bp+"' class='fm-button ui-state-default-grid ui-corner-left'><span class='ui-icon-os ui-icon-triangle-1-w'></span></a>",
				bN = "<a href='javascript:void(0)' id='"+bn+"' class='fm-button ui-state-default-grid ui-corner-right'><span class='ui-icon-os ui-icon-triangle-1-e'></span></a>",
				bC  ="<a href='javascript:void(0)' id='cData' class='fm-button ui-state-default-grid ui-corner-all-grid'>"+p.bClose+"</a>";
				if(maxRows >  0) {
					var sd=[];
					$.each($(tbl)[0].rows,function(i,r){
						sd[i] = r;
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+IDs.themodal).html() !== null ) {
				...
				// buttons at footer
				tbl += "</tbody></table></div>";
				var bS  = "<a href='javascript:void(0)' id='dData' class='fm-button ui-state-default-grid ui-corner-all-grid'>"+p.bSubmit+"</a>",
				bC  = "<a href='javascript:void(0)' id='eData' class='fm-button ui-state-default-grid ui-corner-all-grid'>"+p.bCancel+"</a>";
		...
		return this.each(function() {
			...
			for(var i = 0; i<clone; i++) {
				...
				if (o.add) {
					...
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon-os "+o.addicon+"'></span>"+o.addtext+"</div>");
				...
				if (o.edit) {
					tbd = $("<td class='ui-pg-button ui-corner-all-grid'></td>");
					pEdit = pEdit || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon-os "+o.editicon+"'></span>"+o.edittext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.view) {
					tbd = $("<td class='ui-pg-button ui-corner-all-grid'></td>");
					pView = pView || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon-os "+o.viewicon+"'></span>"+o.viewtext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.del) {
					tbd = $("<td class='ui-pg-button ui-corner-all-grid'></td>");
					pDel = pDel || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon-os "+o.delicon+"'></span>"+o.deltext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.search) {
					tbd = $("<td class='ui-pg-button ui-corner-all-grid'></td>");
					pSearch = pSearch || {};
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon-os "+o.searchicon+"'></span>"+o.searchtext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.refresh) {
					tbd = $("<td class='ui-pg-button ui-corner-all-grid'></td>");
					$(tbd).append("<div class='ui-pg-div'><span class='ui-icon-os "+o.refreshicon+"'></span>"+o.refreshtext+"</div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
				...
				tdw = $(".ui-jqgrid").css("font-size") || "11px";
				$('body').append("<div id='testpg2' class='ui-jqgrid ui-widget-grid ui-widget-content-grid' style='font-size:"+tdw+";visibility:hidden;' ></div>");
		...
		return this.each(function() {
			...
			if (findnav) {
				...
				if(p.buttonicon.toString().toUpperCase() == "NONE") {
                    $(tbd).addClass('ui-pg-button ui-corner-all-grid').append("<div class='ui-pg-div'>"+p.caption+"</div>");
				} else	{
					$(tbd).addClass('ui-pg-button ui-corner-all-grid').append("<div class='ui-pg-div'><span class='ui-icon-os "+p.buttonicon+"'></span>"+p.caption+"</div>");
				}
...
addSubGridCell :function (pos,iRow) {
	...
	return "<td role=\"grid\" aria-describedby=\""+sid+"_subgrid\" class=\"ui-sgcollapsed sgcollapsed\" "+prp+"><a href='javascript:void(0);'><span class='ui-icon-os "+ic+"'></span></a></td>";
},
	...
	return this.each(function(){
		...
		var subGridXml = function(sjxml, sbid){
			...
			for (i = 0; i<ts.p.subGridModel[0].name.length; i++) {
				tddiv = $("<th class='ui-state-default-grid ui-th-subgrid ui-th-column ui-th-"+ts.p.direction+"'></th>");
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						if(ts.p.subGridOptions.reloadOnExpand === true || ( ts.p.subGridOptions.reloadOnExpand === false && !$(r).hasClass('ui-subgrid') ) ) {
							...
							if(bfsc === false) {return false;}
								$(tr).after( "<tr role='row' class='ui-subgrid'>"+atd+"<td class='ui-widget-content-grid subgrid-cell'><span class='ui-icon-os "+ts.p.subGridOptions.openicon+"'></span></td><td colspan='"+parseInt(ts.p.colNames.length-1-nhc,10)+"' class='ui-widget-content-grid subgrid-data'><div id="+pID+"_"+_id+" class='tablediv'></div></td></tr>" );
						...
						$(this).html("<a href='javascript:void(0);'><span class='ui-icon-os "+ts.p.subGridOptions.minusicon+"'></span></a>").removeClass("sgcollapsed").addClass("sgexpanded");
						...
						if(bfsc===false) {return false;}
							...
							$(this).html("<a href='javascript:void(0);'><span class='ui-icon-os "+ts.p.subGridOptions.plusicon+"'></span></a>").removeClass("sgexpanded").addClass("sgcollapsed");
					}
					return false;
				});
	...
	setTreeNode : function(i, len){
		return this.each(function(){
			...
			while(i<len) {
				...
				twrap += "<div style='"+($t.p.direction=="rtl" ? "right:" : "left:")+(lftpos*18)+"px;' class='ui-icon-os ";
	...
	groupingRender : function (grdata, colspans ) {
		return this.each(function(){
			...
			$.each(grp.sortitems[0],function(i,n){
				hid = $t.p.id+"ghead_"+i;
				icon = "<span style='cursor:pointer;' class='ui-icon-os "+pmrtl+"' onclick=\"jQuery('#"+$t.p.id+"').jqGrid('groupingToggle','"+hid+"');return false;\"></span>";
				try {
