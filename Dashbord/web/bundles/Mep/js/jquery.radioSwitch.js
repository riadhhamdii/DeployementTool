...
(function($){
    jQuery.fn.radioSwitch = function(options, switched_callback) {
    	...
    	return this.each(function() {
    		...
    		container.find('label').click(function() {
    		    ...
    		    // Animate handle
                    slide_handle(myHandle, position_left, offset, settings.speed, switched_callback, myLabel.html());
    ...
    /*** Private functions ***/
    function slide_handle(handle, left_pos, right_pos, speed, switch_callback, label)
    {
        jQuery(handle).animate({left: left_pos, right: right_pos}, speed, function() {
                $(this).prev('.radioSwitch-handle').remove();
                $(this).html(label);
